...
void renderMesh() {
    ...
    for(f_it = mesh.faces_begin(); f_it != mesh.faces_end(); ++f_it) {
        ...
        Mesh::Point normal = mesh.calc_face_normal(*f_it);
        glNormal3d(normal[0], normal[1], normal[2]);
        for(; fv_it.is_valid(); ++fv_it) {
            Mesh::Point pt = mesh.point(*fv_it);
...
void glSetMaterial() {
    static float ambient[]  = { 0.2472f, 0.1995f, 0.0745f, 1.0f };
    static float diffuse[]  = { 0.7516f, 0.6064f, 0.2264f, 1.0f };
    static float specular[] = { 0.6282f, 0.5558f, 0.3660f, 1.0f };
    static float shineness  =  51.2f;
